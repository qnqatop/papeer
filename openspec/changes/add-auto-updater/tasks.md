...
- [x] 1.5 Реализовать `ExtractArchive(archivePath, destDir string)` — распаковка `.zip` (через `archive/zip`) и `.tar.gz` (через `archive/tar` + `compress/gzip`) в стейджинг-поддиректорию; проверка, что извлечённый бинарник/бандл существует и не пустой
...
- [x] 1.7.1 Реализовать `installMacOS(stagingDir string) error` — найти текущий `.app`-бандл (от `os.Executable()` подняться до `Papeer.app`), переместить старый бандл в `~/.Trash/`, скопировать новый из стейджинга на исходное место, запустить новый процесс через `os.StartProcess` → `<new.app>/Contents/MacOS/papeer`, затем `os.Exit(0)`
...
- [x] 1.9.1 Реализовать `installLinux(stagingDir string) error` — получить путь текущего бинарника через `os.Executable()`, `os.Rename` нового бинарника поверх старого, вызвать `syscall.Exec` с теми же `os.Args` для подмены процесса на месте
...
- [x] 3.2 Добавить секцию «Обновления» во вкладку «О программе» в SettingsView: отображение текущей версии, кнопка «Проверить обновления», текст статуса, прогресс-бар загрузки (Naive UI `n-progress` или `n-progress-bar`), кнопка «Перезапустить для обновления»
- [x] 3.3 Реализовать реактивное состояние (`updateStatus`, `latestVersion`, `downloadProgress`, `isDownloading`, `isUpdateReady`) в компоненте SettingsView
...
- [x] 4.25 Unit-тест компонента панели обновлений: проверить что в исходном состоянии кнопка «Проверить обновления» видна, прогресс-бар и кнопка перезапуска скрыты
- [x] 4.26 Unit-тест: после установки `isUpdateReady = true` кнопка «Перезапустить для обновления» видна
- [x] 4.27 Unit-тест: во время `isDownloading = true` кнопки заблокированы (disabled)
