<template>
  <n-modal v-model:show="visible" preset="card" :title="t('download.title')" style="width: 640px; max-height: 80vh">
    ...
    <!-- Per-paper log (aggregated from event stream so parallel workers
         don't interleave). One card per paper, sorted by start order. -->
    <n-scrollbar v-if="paperCards.length > 0" style="max-height: 360px" ref="scrollRef">
      <div
...
import {
  NModal, NButton, NSpace, NProgress, NText, NAlert, NScrollbar,
  useMessage,
} from 'naive-ui'
